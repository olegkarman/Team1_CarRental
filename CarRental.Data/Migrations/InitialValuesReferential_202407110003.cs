...
{
    [Migration(202407110003)]

    public class InitialValuesReferential_202407110003 : Migration
    {
		                    ...
		                    WHERE CONSTR_COLUMN.TABLE_NAME = 'Cars'
			                    AND CONSTR_COLUMN.COLUMN_NAME = 'CustomerId')
				                    BEGIN
					                    ...
					                    PRINT 'MRIGRATION IS SUCCESSFULL: FOR TABLE Cars COLUMN CustomerId CREATED FOREIGN KEY';
				                    END
                    ELSE
	                    PRINT 'MRIGRATION FAILED: IN TABLE Cars COLUMN CustomerId SOME FOREIGN KEY ALREADY EXISTS';
                "
            );
        }
					...
					IF EXISTS
						(SELECT TABLE_NAME, CONSTRAINT_NAME
							...
							WHERE TABLE_NAME = 'Cars'
								AND CONSTRAINT_NAME = 'FK_Cars_Users_CustomerId_IdNumber')
									BEGIN
										ALTER TABLE Cars
											DROP CONSTRAINT FK_Cars_Users_CustomerId_IdNumber;
					
										PRINT 'MRIGRATION IS SUCCESSFULL: FOR TABLE Cars COLUMN CustomerId DROPPED FOREIGN KEY';
									END
					ELSE
						PRINT 'MRIGRATION FAILED: FOR TABLE Cars COLUMN CustomerId FOREIGN KEY FK_Cars_Users_CustomerId_IdNumber IS NOT EXIST';
                "
            );
        }
    }
