...
{
    [Migration(202407110007)]

    public class InitialValuesReferential_202407110007 : Migration
    {
                    ...
                    IF NOT EXISTS
	                    (SELECT REF_CONSTRAINT.CONSTRAINT_NAME AS FK_NAME, CONSTR_COLUMN.TABLE_NAME AS SOURCE_TABLE,
		                    ...
		                    WHERE CONSTR_COLUMN.TABLE_NAME = 'Inspections'
			                    AND CONSTR_COLUMN.COLUMN_NAME = 'InspectorId')
				                    BEGIN
					                    ...
					                    PRINT 'MRIGRATION IS SUCCESSFULL: FOR TABLE Inspections COLUMN InspectorId CREATED FOREIGN KEY';
				                    END
                    ELSE
	                    PRINT 'MRIGRATION FAILED: IN TABLE Inspections COLUMN InspectorId SOME FOREIGN KEY ALREADY EXISTS';
                "
            );
        }
					...
					IF EXISTS
						(SELECT TABLE_NAME, CONSTRAINT_NAME
							...
							WHERE TABLE_NAME = 'Inspections'
								AND CONSTRAINT_NAME = 'FK_Inspections_Users_InspectorId_IdNumber')
									BEGIN
										ALTER TABLE Inspections
											DROP CONSTRAINT FK_Inspections_Users_InspectorId_IdNumber;
					
										PRINT 'MRIGRATION IS SUCCESSFULL: FOR TABLE Inspections COLUMN InspectorId DROPPED FOREIGN KEY';
									END
					ELSE
						PRINT 'MRIGRATION FAILED: FOR TABLE Inspections COLUMN InspectorId FOREIGN KEY FK_Inspections_Users_InspectorId_IdNumber IS NOT EXIST';
                "
            );
        }
    }
